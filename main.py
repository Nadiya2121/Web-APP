...
async def web_ui():
    ...
    <body onclick="closeMenu(event)">
        ...
        <div id="adScreen" class="ad-screen">
            ...
            <div class="timer-ui" id="timerUI">
                ...
                <p style="color: #fbbf24; font-size: 18px; font-weight: bold; margin-top:15px;">সার্ভারের সাথে কানেক্ট হচ্ছে...</p>
            </div>
    ...
    """
    html_code = html_code.replace("{{ZONE_ID}}", zone_id).replace("{{TG_LINK}}", tg_url).replace("{{LINK_18}}", link_18).replace("{{AD_COUNT}}", str(required_ads)).replace("{{BOT_USER}}", BOT_USERNAME).replace("{{BKASH_NO}}", bkash_no).replace("{{NAGAD_NO}}", nagad_no)
    return html_code
